...
// options.h is generated by wolfSSL's own configure/cmake, recording the feature flags that
// build used -- only relevant for SITL's pkg-config-detected system install. The vendored
// (WOLFSSL_USER_SETTINGS) build bypasses that whole process; user_settings.h covers everything
// options.h would otherwise, and no options.h file exists to find in that case.
...
# endif // AP_DDS_GPS_GLOBAL_ORIGIN_PUB_ENABLED
    ...
    // outgoing raw MAVLink bytes (mirrored from MAVLINK_COMM_0, see GCS_MAVLink.cpp's
    // comm_send_buffer()), wrapped one filemsg sample per mirrored chunk
    filemsg_msgs_msg_filemsg vehicle_data_pub_topic;
    uint32_t vehicle_data_pub_sample_id;
    //! @brief Serialize the current vehicle_data_pub_topic and publish to the IO stream(s)
    void write_vehicle_data_topic();

    // [YYIL] New. Lock-free single-producer/single-consumer hand-off between the main vehicle
    // scheduler thread (producer, via publish_vehicle_data()) and the dedicated DDS thread
    // (consumer, drained from inside update() where csem is already held). Replaces an earlier
    // csem.take(5)-with-drop-on-timeout mitigation for the same cross-thread contention (see
    // project_dds_mav_mode_semaphore_starvation memory) -- that fix could still, in principle,
    // drop a chunk under bad timing; this one structurally can't, since the producer never
    ...
    // incoming raw MAVLink bytes wrapped in filemsg -- decoded byte-by-byte with a dedicated
    // persistent accumulator (separate from any real UART channel's, since this data never
    // passes through a channel's own byte stream) and dispatched via GCS_MAVLINK::handle_message()
    // as if received on MAVLINK_COMM_0 once a full packet is framed, see on_topic()
    static filemsg_msgs_msg_filemsg rx_vehicle_data_topic;
    static mavlink_message_t vehicle_data_accum_msg;
    static mavlink_status_t vehicle_data_accum_status;
    static mavlink_message_t vehicle_data_rx_msg;
    static mavlink_status_t vehicle_data_rx_status;
#endif // AP_DDS_VEHICLE_DATA_SUB_ENABLED

#if AP_DDS_LOG_REQUEST_SUB_ENABLED
    // incoming MngData Bin(dataflash log) request packets wrapped in filemsg -- separate topic
    // and separate decoder state from VEHICLE_DATA_SUB's GCS command channel, and deliberately
    // dropped while armed (see on_topic()) so a log-download request can never compete with
    // real-time control traffic mid-flight.
    static filemsg_msgs_msg_filemsg rx_log_request_topic;
    static mavlink_message_t log_request_accum_msg;
    static mavlink_status_t log_request_accum_status;
    static mavlink_message_t log_request_rx_msg;
    static mavlink_status_t log_request_rx_status;
    ...
    struct {
        ...
        WOLFSSL_CTX *tls_ctx;
        WOLFSSL *tls;
        // per-call read timeout budget consulted by serial_tls_recv_cb
        uint32_t io_timeout_ms;
#endif
    } serial;

#if AP_DDS_SERIAL_TLS_ENABLED
    // wolfSSL TLS (not DTLS -- serial is a continuous framed byte stream, not a datagram
    // transport) I/O callbacks, bridging to serial.port. Mirrors AP_DDS_UDP.cpp's DTLS callbacks.
    static int serial_tls_send_cb(WOLFSSL *ssl, char *buf, int sz, void *ctx);
    static int serial_tls_recv_cb(WOLFSSL *ssl, char *buf, int sz, void *ctx);
    // run the (blocking) TLS handshake on an already-open serial.port
    bool serial_tls_handshake() WARN_IF_UNUSED;
#endif
    ...
    struct {
        ...
        WOLFSSL_CTX *tls_ctx;
        WOLFSSL *tls;
        // per-call read timeout budget consulted by dtls_recv_cb
        uint32_t io_timeout_ms;
#endif
    } udp;

#if AP_DDS_DTLS_ENABLED
    // wolfSSL DTLS I/O callbacks, bridging to udp.socket
    ...
    bool dtls_handshake() WARN_IF_UNUSED;
#endif
#endif
...
#if AP_DDS_DTLS_ENABLED || AP_DDS_SERIAL_TLS_ENABLED
    //! @brief Enable wolfSSL encryption (X.509 mutual TLS/ECC) on whichever transport is
    //  actually in use -- DTLS if UDP, TLS if Serial. One switch regardless of transport.
    AP_Int8 dtls_enable;
#endif

#if AP_DDS_VEHICLE_DATA_PUB_ENABLED
    //! @brief 0=Normal (MAVLink only, no DDS), 1=DDS-only (MAVLINK_COMM_0's bytes go over DDS
    //  instead of its own transport -- see GCS_MAVLink.cpp's comm_send_buffer() hook)
    AP_Int8 mav_mode;
#endif // AP_DDS_VEHICLE_DATA_PUB_ENABLED
